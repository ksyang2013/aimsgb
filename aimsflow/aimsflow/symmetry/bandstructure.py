import numpy as np
from math import ceil, cos, sin, tan, pi
from warnings import warn

from aimsflow.symmetry.analyzer import SpacegroupAnalyzer
...
class HighSymmKpath(object):
    def __init__(self, structure, symprec=0.01, angle_tolerance=5, atol=1e-8):
        ...
        if not np.allclose(self._structure.lattice.matrix,
                           self._prim.lattice.matrix, atol=atol):
            warn("The input structure does not match the expected standard "
                 "primitive! The path can be incorrect. Use at your own risk.")
        ...
        if not self._kpath:
            raise RuntimeError("Cannot find high symmetry kpoints for this %s "
    ...
    def get_kpoints(self):
        ...
        for b in self.kpath['path']:
            for i in range(1, len(b)):
                ...
                list_k_points.extend([start, end])
                sym_point_labels.extend([b[i - 1], b[i]])
